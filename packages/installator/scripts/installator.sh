...
setup_grub () {
  ...
  append_grub_conf $1 "SifBoX $VERSION " "" "" ""
  append_grub_conf $1 "SifBoX $VERSION (debug)" "" debugging ""
  append_grub_conf $1 "SifBoX $VERSION (reconfigure)" "" "" configure
...
install_grub (){
  ...
  [ -f "$BOOTDISK_MNT/SIFBOX/usr/share/grub-i386-pc.tar.lzma" ] \
    && tar xaf "$BOOTDISK_MNT/SIFBOX/usr/share/grub-i386-pc.tar.lzma" -C /usr \
    >> $LOGFILE 2>&1

  [ -f "$BOOTDISK_MNT/SIFBOX/usr/share/${SPLASHIMAGE}" ] && cp -f "$BOOTDISK_MNT/SIFBOX/usr/share/${SPLASHIMAGE}" $GRUBDIR 
  ...
  # Detect others OS and ask for MBR only in the case where SifBoX
...
'
  ...
  # install to boot record of sifbox partition either way
...
EOF
  ...
  if [ "$MBR" != "yes" ]; then
    # fixme: i18n strings should be updated. in fact, if not installed into mbr, sifbox is installed into respective partition.
...
VERSION=`cat VERSION`
BACKTITLE="SifBoX $VERSION installator"
...
rm -rf $BOOTDISK_MNT/SIFBOX 2>&1 >> $LOGFILE
...
for e in $EMPTY_DIRS; do
  mkdir -p $BOOTDISK_MNT/SIFBOX/$e
done
for e in $DISK_ELEMS; do
  find $e -xdev | sed 's/^\///g' | cpio -pd $BOOTDISK_MNT/SIFBOX 2>&1 | grep -v "newer or same age file exists" >> $LOGFILE
done
...
if [ -n "$CDROM" ] ; then 
 ...
 cp /mnt/cd/SIFBOX/boot/vmlinuz $BOOTDISK_MNT/boot/
 umount /mnt/cd
else
...
# opkg
mkdir -p $BOOTDISK_MNT/SIFBOX/var/lib/opkg/info
cp /var/lib/opkg/info/* $BOOTDISK_MNT/SIFBOX/var/lib/opkg/info
cp /var/lib/opkg/status $BOOTDISK_MNT/SIFBOX/var/lib/opkg/        
...
# Softlink grub.cfg
rm -f $BOOTDISK_MNT/SIFBOX/etc/grub/grub.cfg
ln -s /boot/grub/grub.cfg $BOOTDISK_MNT/SIFBOX/etc/grub/grub.cfg

# Remove unneeded boot dir from mounted install drive
rm -rf $BOOTDISK_MNT/SIFBOX/boot
